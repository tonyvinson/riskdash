    ...
    def publish_text_message(self, phone_number, message):
        ...
        try:
            response = self.sns_resource.publish(
                # PhoneNumber=os.getenv("AWS_SNS_ORIGINATION_NUMBER"),
                PhoneNumber=phone_number,
                Message=message,
                # TargetArn=os.getenv("AWS_SNS_TOPIC_ARN"),
            )
...
def usage_demo():
    ...
    email = input(
        f"Enter an email address to subscribe to {topic_name} and receive " f"a message: "
    )
    ...
    if phone_sub is not None:
        ...
        sns_wrapper.publish_message(
            topic,
            "Hey. This message is not mobile friendly, so you shouldn't get " "it on your phone.",
